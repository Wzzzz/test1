...
while(!isallfinished(isfinish))//反复调度直到所有节点都执行完毕
{
  ...
  int selectedid;
  int dl[10]={-1024,-1024,-1024,-1024,-1024,-1024,-1024,-1024,-1024,-1024};
  for(int i=1;i<10;i++)//更新就绪列表
  {
    ...
    if(flag&&(isfinish[i]==0))//当该节点所有父节点都调度，并且该节点未被调度时，该节点标记为就绪节点
    {
    isready[i]=1;
    }else
    {
...
//----------------------------------------------------------------------------------------------------------------trymap---
 ...
 trymap[selectedid]=minitem(early);
 trystarttime[selectedid]=early[minitem(early)];
...
int k1;
//====================================================================================================取最大DL
for(int i=1;i<10;i++)//求出所有就绪节点的DL值，节点DL值=节点SBL值-节点最早开始时间
{
if(isready[i]==1)
  dl[i]=SBL[i]-trystarttime[i];
}
  int k=dl[9]>dl[8]?9:8;
  k=dl[k]>dl[7]?k:7;
  k=dl[k]>dl[6]?k:6;
  ...
  k=dl[k]>dl[3]?k:3;
  k=dl[k]>dl[2]?k:2;
  k=dl[k]>dl[1]?k:1;//取具有最大DL值的节点
k1=k;//k1的值为所有就绪节点中，具有最大DL节点-处理机对的编号，
//====================================================================================================
int fangan=trymap[k1];//记录应该分配到哪台机器（开始时间最早）
  ...
  time[fangan]=pe[k1]+t[k1];//该机器（fangan）执行该任务（k1）后的时间
  isfinish[k1]=1;//将该任务标记为已调度
